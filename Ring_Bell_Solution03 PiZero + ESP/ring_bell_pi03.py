    ...
    if data.get("topic") == "/tracked_pose":
        ...
        if robot_ip in robots_status:
            ...
            if robot['current_target_x'] is not None and robot['current_target_y'] is not None:
                ...
                if new_arrive_stat != robot['arrive_stat_calc']:
                    robot['arrive_stat'] = new_arrive_stat
                    robot['arrive_stat_calc'] = new_arrive_stat
        ...
        if latest_move:
            ...
            if move_id != robots_status[robot_ip]['current_move_id']:
                ...
                if move_details:
                    ...
                    robots_status[robot_ip]['arrive_stat'] = "arrived" if state == "succeeded" else "in_progress"
            ...
            function restartService() {
                fetch('/restart_service', { method: 'POST' })
                    ...
                    .then(data => {
                        ...
                        fetchStatus(); // Refresh JSON data after restart
                    })
                    .catch(error => console.error('Error:', error));
            }

            window.onload = fetchStatus; // Load JSON data when the page loads
        </script>
    </head>
    <body>
        <h1>Robot Status</h1>
...
def restart_service():
    try:
        subprocess.run(["sudo", "systemctl", "restart", "ring_bell_esp.service"], check=True)
