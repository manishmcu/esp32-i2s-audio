...
def on_message(ws, message):
    global current_pos_x, current_pos_y, current_move_id, current_target_x, current_target_y, robot_ip, arrive_stat, arrived
    ...
    if data.get("topic") == "/tracked_pose":
        ...
        if current_target_x is not None and current_target_y is not None:
            ...
            # Print status with distance (only print "near" or "arrived")
...
def check_for_new_move():
    global current_move_id, current_target_x, current_target_y, current_pos_x, current_pos_y, robot_ip, arrive_stat, arrived
    while True:
        # Fetch the latest move command
        ...
        # Sleep before checking for new move commands
        time.sleep(1)
